...
function turbulence_perlin(m, r, theta, a, M, lum, correlation_length, mach; noise_r = perlin_2d(seed=1), noise_p = perlin_2d(seed=2), noise_a = perlin_2d(seed=3))
    ...
    noise_val_r = (1/sqrt(3))*mach*c(m, r, a, M, lum)*sample(noise_r, x/correlation_length, y/correlation_length)
    noise_val_p = (1/sqrt(3))*mach*c(m, r, a, M, lum)*sample(noise_p, x/correlation_length, y/correlation_length)
    noise_val_a = (1/sqrt(3))*mach*c(m, r, a, M, lum)*sample(noise_a, x/correlation_length, y/correlation_length)
